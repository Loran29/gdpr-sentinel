...
import { owner_summaries as mock_owner_summaries } from "@/lib/mock-data";
import { get_admin_owners } from "@/src/lib/api-client";
import { OwnerSummary } from "@/types/models";

export function DataOwnersPage() {
  const [owners, set_owners] = useState<OwnerSummary[]>(mock_owner_summaries);

  useEffect(() => {
    let is_cancelled = false;
    const load = async () => {
      const result = await get_admin_owners();
      if (!is_cancelled && result.length > 0) {
        set_owners(result);
      }
    };
    load();
  ...
  const orphan_count = owners.filter((o) => o.assigned_sources.includes("**")).length;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        ...
        <SummaryCard title="Orphan files routed to default Master of Data" value={orphan_count} />
      </div>
